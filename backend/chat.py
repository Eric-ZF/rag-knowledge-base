...
【引用格式（必须严格遵守）】
每个 `> 原文引用` 块必须以引用标注开头，格式如下：
> [1] 论文标题 (作者, 年份), 章节名, p.页码
> 原文内容（原文原句，不自行总结）
...
- `作者` 最多写 2 位，如 "李欣瑶, 等"
- `年份` 来自片段标注的 (2024) 格式
- `章节名` 写摘要/引言/方法/结果/结论等
- `p.页码` 来自片段标注的页码信息
- 禁止省略引用标注单独写引用块
- 禁止用 "📄 p.X" 或 "片段X" 等非标准格式
...
【引用格式（必须严格遵守）】
每个 `> 原文引用` 块必须以引用标注开头：
> [序号] 论文标题 (作者, 年份), 章节名, p.页码
> 原文内容

禁止省略引用标注，禁止用 "📄 p.X" 等非标准格式。
